...
exports.seed = async (knex) => {
  ...
  for (var i = 1; i <= scaledData; i++) {
    var id = i;
    var dataToSave = { song_id: id };
    dataToSave.song_length = Math.floor(Math.random() * (max - min) + min);
    dataToSave.song_name = names[Math.floor(Math.random() * ((names.length - 1)))];
    dataToSave.song_url = 'https://rpt23-fec-soundcloud.s3-us-west-2.amazonaws.com/Djenty+Metal+Town%2C+USA.mp3'
    dataToSave.song_image = photos[Math.floor(Math.random() * ((photos.length - 1)))];
    // dataToSave.bandID = Math.floor(Math.random() * (30 - 1) + 1);
    var saved = await knex('songs').insert(dataToSave);
  }
};
